...
export class MortalActorSheet extends HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2) {
	
	/** @override */
	// static get defaultOptions() {
	// 	return foundry.utils.mergeObject(super.defaultOptions, {
	// 		classes: ["wod20", "wod-sheet", "mortal"],
	// 		template: "systems/worldofdarkness/templates/actor/mortal-sheet.hbs",
	// 		width: 800,
	// 		height: "auto",
	// 		resizable: true,
	// 		scrollY: [".sheet-body", ".sheet-setting-body"]
	// 	});
	// }

	static DEFAULT_OPTIONS = {
		classes: ["wod20", "wod-sheet", "mortal", "actor"],
		window: {
	...
	activateListeners(html) {
		...
		// lock button
		html.querySelector(".lock-btn")?.addEventListener("click", this._onToggleLocked.bind(this));
		...
		async _onDropItem(_event, _data) {
			...
			if (itemsList[itemType]) {
				...
				if (!foundry.utils.isEmpty(whitelist) && whitelist.indexOf(actorType) === -1) {
					ui.notifications.warn('LANG: Item can not be dropped on Actor', {
						string1: itemType,
						string2: actorType
					})
